...
const (
	_1_DOT_1 EnumQueryDouble = 1.1
	_MINUS_1_DOT_2 EnumQueryDouble = -1.2
)

var allowedEnumQueryDoubleEnumValues = []EnumQueryDouble{
	1.1,
	-1.2,
}
...
func (v *EnumQueryDouble) UnmarshalJSON(src []byte) error {
	...
	for _, existing := range allowedEnumQueryDoubleEnumValues {
...
// NewEnumQueryDoubleFromValue returns a pointer to a valid EnumQueryDouble
// for the value passed as argument, or an error if the value passed is not allowed by the enum
func NewEnumQueryDoubleFromValue(v float64) (*EnumQueryDouble, error) {
	ev := EnumQueryDouble(v)
	if ev.IsValid() {
		return &ev, nil
	} else {
		return nil, fmt.Errorf("invalid value '%v' for EnumQueryDouble: valid values are %v", v, allowedEnumQueryDoubleEnumValues)
	}
}

// IsValid return true if the value is valid for the enum, false otherwise
func (v EnumQueryDouble) IsValid() bool {
	for _, existing := range allowedEnumQueryDoubleEnumValues {
		if existing == v {
			return true
		}
