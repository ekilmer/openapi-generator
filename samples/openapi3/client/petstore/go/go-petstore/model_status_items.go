/*
OpenAPI Petstore

This spec is mainly for testing Petstore server and contains fake endpoints, models. Please do not use this for any other purpose. Special characters: \" \\

API version: 1.0.0
*/

// Code generated by OpenAPI Generator (https://openapi-generator.tech); DO NOT EDIT.

package petstore

import (
	"encoding/json"
	"fmt"
)

// StatusItems the model 'StatusItems'
type StatusItems string

// List of statusItems
const (
	STATUS_ITEMS_AVAILABLE StatusItems = "available"
	STATUS_ITEMS_PENDING StatusItems = "pending"
	STATUS_ITEMS_SOLD StatusItems = "sold"
)

var allowedStatusItemsEnumValues = []StatusItems{
	"available",
	"pending",
	"sold",
}

func (v *StatusItems) UnmarshalJSON(src []byte) error {
	var value string
	err := json.Unmarshal(src, &value)
	if err != nil {
		return err
	}
	enumTypeValue := StatusItems(value)
	for _, existing := range allowedStatusItemsEnumValues {
		if existing == enumTypeValue {
			*v = enumTypeValue
			return nil
		}
	}

	return fmt.Errorf("%+v is not a valid StatusItems", value)
}

// NewStatusItemsFromValue returns a pointer to a valid StatusItems
// for the value passed as argument, or an error if the value passed is not allowed by the enum
func NewStatusItemsFromValue(v string) (*StatusItems, error) {
	ev := StatusItems(v)
	if ev.IsValid() {
		return &ev, nil
	} else {
		return nil, fmt.Errorf("invalid value '%v' for StatusItems: valid values are %v", v, allowedStatusItemsEnumValues)
	}
}

// IsValid return true if the value is valid for the enum, false otherwise
func (v StatusItems) IsValid() bool {
	for _, existing := range allowedStatusItemsEnumValues {
		if existing == v {
			return true
		}
	}
	return false
}

// Ptr returns reference to statusItems value
func (v StatusItems) Ptr() *StatusItems {
	return &v
}

type NullableStatusItems struct {
	value *StatusItems
	isSet bool
}

func (v NullableStatusItems) Get() *StatusItems {
	return v.value
}

func (v *NullableStatusItems) Set(val *StatusItems) {
	v.value = val
	v.isSet = true
}

func (v NullableStatusItems) IsSet() bool {
	return v.isSet
}

func (v *NullableStatusItems) Unset() {
	v.value = nil
	v.isSet = false
}

func NewNullableStatusItems(val *StatusItems) *NullableStatusItems {
	return &NullableStatusItems{value: val, isSet: true}
}

func (v NullableStatusItems) MarshalJSON() ([]byte, error) {
	return json.Marshal(v.value)
}

func (v *NullableStatusItems) UnmarshalJSON(src []byte) error {
	v.isSet = true
	return json.Unmarshal(src, &v.value)
}

